"""
CAST Evaluation Script for SLAKE Dataset (Phi-3.5V-Med)

Evaluates CAST (Contrastive Anatomical Spatial-Temporal decoding)
on the SLAKE medical VQA dataset.

Usage:
    python evaluate_slake.py --mode auto
    python evaluate_slake.py --mode gt      # Use GT masks (upper bound)
    python evaluate_slake.py --mode null    # No mask (baseline)
"""
...
class SLAKEDataset:
    ...
    def _get_mask_value(self, mask_img, bbox):
        if bbox is None:
            return 0
        mask_np = np.array(mask_img)
        x, y, w, h = map(int, bbox)
        cx, cy = x + w // 2, y + h // 2
        if 0 <= cy < mask_np.shape[0] and 0 <= cx < mask_np.shape[1]:
            return int(mask_np[cy, cx])
        return 0
    ...
    def __getitem__(self, index):
        ...
        mask_value = self._get_mask_value(image_mask, bbox)

        question = ann['question']
        answer = ann['answer']
        answer_type = ann.get('answer_type', 'OPEN').upper()

        highlight = [obj_name] if obj_name else ([answer] if answer else [])
        ...
        return {
            ...
            "image_path": image_path, "highlights": highlight,
...
def generate_bbox_mask(mask_crop, n_rows, n_cols, block_dims=(12, 12)):
    ...
    local_mask = F.interpolate(
        ...
        size=(target_h_local, target_w_local),
        mode='nearest'
    ).squeeze(1)
    ...
    final_mask = torch.cat([local_seq, separator, global_seq], dim=1)
    return final_mask
...
def create_mask_provider(mode, config, model=None, processor=None, device='cuda'):
    """Create appropriate MaskProvider based on mode."""
    ...
    if mode == 'null':
        ...
        try:
            ...
            print("  - Falling back to SimpleProposalGenerator")
            proposal_gen = SimpleProposalGenerator()

        cache = ProposalCache(cache_dir=config['cache_dir'], prefix='slake')
...
def parse_args():
    parser = argparse.ArgumentParser(description='CAST Evaluation on SLAKE (Phi-3.5V-Med)')

    parser.add_argument('--mode', type=str, default='auto',
                        choices=['auto', 'gt', 'null'],
                        help='Mask mode: auto (CAST), gt (ground truth), null (no mask)')

    # Paths
    parser.add_argument('--model_path', type=str, required=True,
                        help='Path to Phi-3.5-vision-instruct')
    parser.add_argument('--lora_path', type=str, default=None,
                        help='Path to LoRA adapter (Phi-3.5V-Med)')
    parser.add_argument('--input_path', type=str, required=True,
                        help='Path to SLAKE test.json')
    parser.add_argument('--img_root', type=str, required=True,
                        help='Root directory of SLAKE dataset')
    ...
    parser.add_argument('--medsam3_weights', type=str, default=None,
                        help='Path to MedSAM3 LoRA weights')
    parser.add_argument('--medsam3_path', type=str, default=None,
                        help='Path to MedSAM3 repo directory')
    parser.add_argument('--cache_dir', type=str, default='cache/slake_proposals',
                        help='Proposal cache directory')
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"CAST Evaluation on SLAKE")
    ...
    if rank == 0:
        ...
        print(f"\n{'=' * 60}")
        print(f"Results - SLAKE / {args.mode.upper()}")
        print(f"{'=' * 60}")
        ...
        save_path = args.output_file or os.path.join(args.save_dir, f'output_slake_cast_{args.mode}.json')
